import { DEFAULT_RULE, HOUR, SETTINGS_KEY, parseThreshold, validateRule, ruleFor, statusFor } from './domain.js';
...
async function init() {
  ...
  if (!lists.length) { $('connection').textContent = 'Erstelle zuerst eine Liste auf diesem Board.'; return; }
  ...
  try {
    ...
    $('connection').textContent = writable ? 'Verbunden · Einstellungen gelten für dieses Board.' : 'Lesezugriff · Nur Board-Mitglieder können Einstellungen ändern.';
    $('connection').className = 'notice success';
  } catch (error) {
...
$('reset').addEventListener('click', () => { dirty = true; display(DEFAULT_RULE); });
...
$('open-auth').addEventListener('click', () => {
  ...
  t.popup({ title: 'Productivity verbinden', url: './authorize.html', height: 470 });
});
init().catch(() => { $('connection').textContent = 'Einstellungen konnten nicht geladen werden. Öffne diese Ansicht über Productivity in Trello.'; resize(); });
